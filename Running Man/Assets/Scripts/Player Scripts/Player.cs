...
public class Player : MonoBehaviour {
    ...
    protected float groundLevel,            //Change later to use raycasting
                  ...
                  slideTimeMax = 0.4f,      //Max slide time (Distance)
    ...
    protected void Gravity()
    {
        ...
        if (Physics.Raycast(transform.position, downVec, out hit, 1.0f))
        {
            if (hit.collider.gameObject.tag == "Ground")
            {
                ...
                groundLevel = hit.collider.gameObject.transform.position.y + 0.55f;
                //Debug.Log("Grounded at " + groundLevel + " height");
    ...
    void Slide()
    {
        ...
        if (slideTime >= slideTimeMax)
        {
            slideTime = 0.0f;
            isSliding = false;
            myCollider.center.Set(myCollider.center.x, colCY, myCollider.center.z);
            myCollider.size.Set(myCollider.size.x, colSY, myCollider.size.z);
        }
    ...
    public void ButtonPresses(string name)
    {
        ...
        if (Input.GetKeyDown("s"))
        {
            ...
            {
                isSliding = true;
                myCollider.center.Set(myCollider.center.x, colCYSlide, myCollider.center.z);
                myCollider.size.Set(myCollider.size.x, colSYSlide, myCollider.size.z);
